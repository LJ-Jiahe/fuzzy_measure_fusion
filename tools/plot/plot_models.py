...
def plot_models(MSEs_unseen_by_num_source, params, output_dir):
        ...
        for distribution_idx, distr in enumerate(distributions):
            for npp_idx, npp in enumerate(num_per_perm_list_train):
                for avg_idx, avg_name in enumerate(avg_names[num_source]):
                        ...
                        # plt.xticks(x_seen)
                        ax.set_title('Distr=' + distr + ', NumSource=' + str(num_source) + ', NPP=' + str(npp) + ', Avg Func=' + avg_name)
                        ...
                        plt.savefig(output_dir + distr + '-' + 'NumSource=' + str(num_source) + ' NPP=' + str(npp) + ' Avg Func=' + str(avg_name) + '.png')
