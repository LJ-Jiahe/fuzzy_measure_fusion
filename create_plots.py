...
for num_source in num_source_list:
    ...
    for distribution_idx, distr in enumerate(distributions):
        for npp_idx, _ in enumerate(num_per_perm_list_train):
            for model_idx, _ in enumerate(model_names):
                for data_type in ["seen", "unseen"]:
                    ...
                    ax.set_title('MSE (' + data_type + '), Model=' + model_names[model_idx] + ', Distr=' + distr + ', NumSource=' + str(num_source) + ', NPP=' + str(num_per_perm_list_train[npp_idx]))
