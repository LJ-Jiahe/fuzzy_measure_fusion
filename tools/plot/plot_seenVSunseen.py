...
def plot_svsuns(MSEs_seen_by_num_source, MSEs_unseen_by_num_source, params, output_dir, axis_left=0.15, axis_right=0.95, y_limit=[-0.003/20, 0.003]):
            ...
            for npp_idx, npp in enumerate(num_per_perm_list_train):
                for avg_idx, avg_name in enumerate(avg_names[num_source]):
                    for model_idx, model_name in enumerate(model_names):
                        ...
                        # plt.xticks(x_seen)
                        ax.set_title('Model=' + model_name + ', Distribution=' + distr + ', Num of Source=' + str(num_source) + '\nSample Per Permutaion=' + str(npp) + ', Operator=' + avg_name)
                        ...
                        plt.savefig(output_dir + 'Seen VS Unseen ' + model_name + '-' + distr + '-' + 'NumSource=' + str(num_source) + ' NPP=' + str(npp) + ' Operator=' + str(avg_name) + '.png')
