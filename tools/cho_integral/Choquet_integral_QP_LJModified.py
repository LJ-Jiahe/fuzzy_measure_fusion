...
#         sol = solve_qp(2 * E, L, G, h, A, b, solver='cvxopt')
#         g = sol
    ...
    def build_constraint_matrices(self, index_keys, fm_len):
        ...
        G = line 
        line[index_keys[tuple(np.array([1]))]] = -1.
        h = np.array([0.])
        for i in range(2, self.N + 1):
            line = np.zeros(fm_len)
            line[index_keys[tuple(np.array([i]))]] = -1.
            G = np.vstack((G, line))
            ...
            for latt_pt in parent:
                for j in range(len(latt_pt) - 1, len(latt_pt)):
                    ...
                    for latt_ch in children:
                        line = np.zeros(fm_len)
                        line[index_keys[tuple(latt_ch)]] = 1.
                        line[index_keys[tuple(latt_pt)]] = -1.
                        G = np.vstack((G, line))
                        h = np.vstack((h, np.array([0])))

        line = np.zeros(fm_len)
        line[index_keys[tuple(vls)]] = 1.
        G = np.vstack((G, line))
    ...
    def get_fm_class_img_coeff(self, Lattice, h, fm_len):  # Lattice is FM_name_and_index, h is the samples, fm_len
        ...
        pi_i = np.argsort(-h) + 1
        for i in range(1, n):
            fm_coeff[Lattice[tuple(np.sort(pi_i[:i]))]] = h[pi_i[i - 1] - 1] - h[pi_i[i] - 1]
        fm_coeff[Lattice[tuple(np.sort(pi_i[:n]))]] = h[pi_i[n - 1] - 1]
    ...
    def get_keys_index(self):
        ...
        for i in range(0, self.N):
            Lattice[tuple(np.array([vls[i]]))] = count
            count = count + 1
        for i in range(2, self.N + 1):
            A = np.array(list(itertools.combinations(vls, i)))
            for latt_pt in A:
                Lattice[tuple(latt_pt)] = count
                count = count + 1
        return Lattice
